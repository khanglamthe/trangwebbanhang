<?php
	...
	if(isset($_GET['quanly'])&&$_GET['quanly']=='xoa'){
		$id=$_GET['id'];
		$sql_xoa=mysqli_query($conn,"delete from hinhanh where MSHH='$id'");
		header('../gallery/lietke.php?id='.$id);
	}
	//them
		...
		foreach($_FILES['file']['name'] as $i =>$name){
			...
			if(empty($thongbao)){
				if(!file_exists('uploads')){
					...
					$sql=mysqli_query($conn,"insert into hinhanh(MSHH,hinh) value('$id','".$hinhanhsp."')");
					header('../gallery/lietke.php?id='.$id);
				}else{
